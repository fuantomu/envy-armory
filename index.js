...
const enchants = require('./enchants.json')
const affixes = require('./affix.json')
...
async function getSortedEquipment(characterName) {
    return fetch(baseUrl.replace("CHARACTERNAME", characterName.toLowerCase())).then((response) => {
        return response.json().then((character) => {
            ...
            for(item of items){
                ...
                if(!ignore_enchant.includes(slotNames[item["slot"]["type"]])) {
                    if(item["enchantments"]){
                        item["link"] += "&ench="
                        filtered = enchants[slotNames[item["slot"]["type"]]].filter(entry => entry["id"] === item["enchantments"][0]["enchantment_id"])
